...
export const GOVERNMENT = {
  ...
  governmentPhotos: (id: string) => `${GOVERNMENT_BASE}/complaints/${id}/government-photos`,
} as const;
...
export type ComplaintItem = {
  ...
  status?: string;
  createdAt?: string;
  /** Note/description added by government when they verify or resolve */
  governmentNotes?: string;
  /** Proof photos uploaded by government (verification / work done) */
  governmentPhotos?: string[];
};

/** Complaint with populated user and government fields (government API) */
...
export const governmentApi = {
  ...
  updateComplaintStatus(
    ...
    return governmentRequest<GovComplaintItem>(GOVERNMENT.complaintStatus(id), {
      method: "PATCH",
      body: notes != null ? { status, notes } : { status },
    });
  },
  async uploadGovernmentPhotos(
    id: string,
    formData: FormData
  ): Promise<{ data: GovComplaintItem; ok: true } | { error: string; ok: false }> {
    const headers = getAuthHeaders();
    if (!headers.Authorization) {
      return { ok: false, error: "Not logged in" };
    }
    try {
      const res = await fetch(GOVERNMENT.governmentPhotos(id), {
        method: "POST",
        headers,
        body: formData,
      });
      ...
      if (!res.ok) {
        return {
          ok: false,
          error: (data as { message?: string }).message || "Upload failed",
        };
      }
      return { ok: true, data: data as GovComplaintItem };
    } catch (e) {
      return {
        ok: false,
        error: e instanceof Error ? e.message : "Network error",
