...
export default function ProfilePage() {
  ...
  const openTrackingModal = (c: ComplaintItem) => {
    setTrackingComplaint(c);
    setTrackingDetailLoading(true);
    citizenApi.getComplaint(c._id).then((res) => {
      setTrackingDetailLoading(false);
      if (res.ok) setTrackingComplaint(res.data);
    });
  };
                      ...
                      {complaints.length === 0 ? (
                        ...
                        complaints.map((c) => (
                          <Row
                            ...
                            location={c.location || "—"}
                            onStatusClick={() => openTrackingModal(c)}
...
onClick={() => { setTrackingComplaint(null); setTrackingDetailLoading(false); }}
                  >
                    <div
                  ...
                  >
                    ...
                    {trackingDetailLoading && (
                      <p className="text-sm text-gray-500 mb-4">Loading latest details…</p>
                    )}
                    ...
                    {/* Government's proof & description – visible to citizen */}
                    {(trackingComplaint.governmentNotes || (trackingComplaint.governmentPhotos && trackingComplaint.governmentPhotos.length > 0)) && (
                      <div className="mt-6 pt-6 border-t border-gray-200">
                        <h4 className="text-sm font-bold text-gray-800 mb-3">Government response (proof & description)</h4>
                        {trackingComplaint.governmentNotes && (
                          <div className="mb-3 p-3 rounded-xl bg-blue-50 border border-blue-100">
                            <p className="text-xs font-semibold text-blue-700 mb-1">Description / note by government</p>
                            <p className="text-sm text-gray-700">{trackingComplaint.governmentNotes}</p>
                          </div>
                        )}
                        {trackingComplaint.governmentPhotos && trackingComplaint.governmentPhotos.length > 0 && (
                          <div>
                            <p className="text-xs font-semibold text-blue-700 mb-2">Proof uploaded by government</p>
                            <div className="flex flex-wrap gap-2">
                              {trackingComplaint.governmentPhotos.map((url, i) => (
                                <img
                                  key={i}
